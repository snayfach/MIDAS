...
def write_summary_stats(species_id, samples, args):
	""" Write summary file for samples """
	outfile = open('%s/%s/%s.genes.samples' % (args['outdir'], species_id, species_id), 'w')
	...
	for sample in samples:
		...
		for field in fields:
			outfile.write(sample.id+'\t%s' % str(stats[species_id][field]))
		outfile.write('\n')
...
def run_pipeline(args):
	...
	for sp in species:
		...
		write_summary_stats(sp.id, sp.samples, args)
